...
function showResult(user) {
    ...
    // ===== SALARY SECTION (القسم الثابت - عمودين) =====
    const salarySection = document.createElement('div');
    salarySection.className = 'salary-card salary-section-fixed';
    salarySection.innerHTML = '<h3 class="section-header"><i class="fas fa-money-bill-wave ml-2" aria-hidden="true"></i> الراتب</h3>';

    const salaryGrid = document.createElement('div');
    salaryGrid.className = 'salary-grid';

    const salaryFields = [
        { key: 'اساسى 30/6', label: 'أساسي 2014', icon: 'fa-coins' },
        { key: 'اساسى يوليو 2025', label: 'أساسي يوليو 2025', icon: 'fa-coins' }
    ];
    ...
    salaryFields.forEach(field => {
        ...
        if (value) {
            ...
            item.className = 'salary-grid-item';
            item.innerHTML = `
                <div class="salary-grid-label">
                    <i class="fas ${field.icon} ml-2 text-blue-500" aria-hidden="true"></i>
                    ${field.label}
                </div>
                <div class="salary-grid-value">${formatCurrency(value)}</div>
            `;
            salaryGrid.appendChild(item);
        }
    });

    salarySection.appendChild(salaryGrid);
    dataContainer.appendChild(salarySection);

    // ===== EMPLOYEE INFO GRID (بيانات الموظف - عمودين) =====
    const infoSection = document.createElement('div');
    infoSection.className = 'info-grid-section';

    const infoFields = [
        { key: 'الادارة', label: 'الإدارة', icon: 'fa-building' },
        { key: 'الوظيفة', label: 'الوظيفة', icon: 'fa-briefcase' },
        { key: 'تاريخ الميلاد', label: 'تاريخ الميلاد', icon: 'fa-birthday-cake' },
        { key: 'تاريخ التعيين', label: 'تاريخ التعيين', icon: 'fa-calendar-check' },
        { key: 'العنوان', label: 'العنوان', icon: 'fa-map-marker-alt' },
        { key: 'رقم التليفون', label: 'رقم التليفون', icon: 'fa-phone' },
        { key: 'البريد', label: 'البريد الإلكتروني', icon: 'fa-envelope' },
        { key: 'الحالة الوظيفية', label: 'الحالة الوظيفية', icon: 'fa-user-check' },
        { key: 'النوع', label: 'النوع', icon: 'fa-venus-mars' },
        { key: 'الديانة', label: 'الديانة', icon: 'fa-pray' },
        { key: 'المؤهل', label: 'المؤهل', icon: 'fa-graduation-cap' },
        { key: 'تاريخ المؤهل', label: 'تاريخ المؤهل', icon: 'fa-calendar' }
    ];

    infoFields.forEach(field => {
        const value = user[normalizeKey(field.key)];
        if (value) {
            const item = document.createElement('div');
            item.className = 'info-grid-item';
            item.innerHTML = `
                <div class="info-grid-label">
                    <i class="fas ${field.icon} ml-2 text-blue-500" aria-hidden="true"></i>
                    ${field.label}
                </div>
                <div class="info-grid-value">${escapeHTML(String(value))}</div>
            `;
    ...
    const excludeFields = [
        ...
        'الادارة', 'الوظيفة', 'تاريخ الميلاد', 'تاريخ التعيين', 'العنوان',
        'رقم التليفون', 'الحالة الوظيفية', 'النوع', 'الديانة', 'المؤهل', 'تاريخ المؤهل'
    ];
