...
function showResult(user) {
    ...
    if (paymentItems.length > 0) {
        ...
        paymentItems.forEach(({ key, value }) => {
            ...
            const isRed = key.includes('#');
            const labelClass = isRed ? 'font-medium text-red-600' : 'font-medium text-slate-700';
            const labelStyle = isRed ? ' style="color:#dc2626;font-weight:700;"' : '';
            row.innerHTML = `
                <span class="${labelClass}"${labelStyle}>${escapeHTML(key)}</span>
